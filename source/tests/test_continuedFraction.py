...
class TestContinuedFracture(TestCase):
    ...
    def test_enumerate_signed_RCF(self):
        ...
        with mpmath.workdps(self.precision):
            ...
            adjusted = [[res[0], res[1], list(res[3])] for res in (results + recurring)]
            self.assertIn([(e/(e-1)), [1, -1], [1, 0, -2, 0, 1]], adjusted)

    def test_create_and_use_generic_lhs(self):
        generic_lhs = create_standard_lhs(poly_deg=1, coefficients_limit=2, out_path=None)
        enumerator = SignedRcfEnumeration(sym_constant=e, cycle_len_range=[2, 2], custom_enum=generic_lhs, do_print=False)
        with mpmath.workdps(self.precision):
            results, duplicates = enumerator.find_hits()
            flatten = (lambda l: [item for sublist in l for item in sublist])
            recurring = flatten([duplicates[key] for key in duplicates.keys()])
