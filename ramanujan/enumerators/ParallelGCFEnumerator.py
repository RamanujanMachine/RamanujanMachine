...
def calculate_RAM_usage(shape):
    ...
    """
    c = 200.0
    return 8.0 * mp.fprod(shape) * 8.0 + c * mp.fsum(shape) * 8.0
...
class ParallelGCFEnumerator(EfficientGCFEnumerator):
    ...
    @staticmethod
    def __create_series_list(
        coefficient_iter: Iterator,
    ...
    def _first_enumeration(self, verbose: bool) -> List[Match]:
        ...
        if min_chunks < max(asize, bsize):  # Iterate over intervals on the longer axis
            ...
            bchunk = (
                1 if asize >= bsize else np.int(np.ceil(asize * bsize / min_chunks))
            )
        ...
        if verbose:
            ...
            print(
                f"Doing {num_iterations} searches in up to {chunks_total} chunks. "
            ...
            for _ in range(0, small_size, small_chunk):
                ...
                for aind in range(shape[0]):
                    for bind in range(shape[1]):
                        ...
                        if (
                            key in self.hash_table
                        ):  # find hits in hash table (bottleneck)
                            results.append(
                                Match(key, a_poly["coef"][aind], b_poly["coef"][bind])
                            )
                    ...
                    if verbose:
                        ...
                        if print_counter >= 1_000_000:  # print status.
                            ...
                            print(
                                f"Passed {counter:n} out of {num_iterations:n} "
                ...
                if verbose:  # Chunk complete
                    ...
                    if prediction < 120:
                        ...
                        prediction = (
                            f"{prediction//3600:.0f}h {prediction%3600//60:.0f}min"
                        )

                    time_left = (time() - start_results) * (
                        num_iterations / counter - 1
                    )
                    if time_left < 120:
                        ...
                        time_left = (
                            f"{time_left//3600:.0f}h {time_left%3600//60:.0f}min"
                        )

                    print(
                        f"Chunk {chunks_done}/{chunks_total} done. "
