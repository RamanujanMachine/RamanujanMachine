...
from ramanujan.utils.utils import (
    find_polynomial_series_coefficients,
    create_mpf_const_generator,
...
RefinedMatch = namedtuple(
    "RefinedMatch", "lhs_key rhs_an_poly rhs_bn_poly lhs_match_idx c_top c_bot"
)
...
class AbstractGCFEnumerator(metaclass=ABCMeta):
    ...
    def __init__(self, hash_table, poly_domains, sym_constants):
        ...
        self.enum_dps = (
            g_N_initial_search_dps  # working decimal precision for first enumeration
        )
        self.verify_dps = (
            g_N_verify_dps  # working decimal precision for validating results.
        )
        ...
        self.create_an_series = lambda coefs, items: get_series_items_from_iter(
            a_iterator_func, coefs, items
        )
        self.create_bn_series = lambda coefs, items: get_series_items_from_iter(
            b_iterator_func, coefs, items
        )
    ...
    def __get_formatted_results(
        self, results: List[RefinedMatch]
        ...
        for r in results:
            ...
            print_length = max(
                max(
                    get_size_of_nested_list(r.rhs_an_poly),
                    get_size_of_nested_list(r.rhs_bn_poly),
                ),
                5,
            )
    ...
    def __get_formatted_polynomials(self, result: RefinedMatch):
        def sym_poly(poly_deg, poly_terms):
            poly = list(
                reversed(find_polynomial_series_coefficients(poly_deg, poly_terms, 0))
            )
            n = sympy.Symbol("n")
            poly_sym = 0
    ...
    def print_results(
        self, results: List[RefinedMatch], formatting="unicode", convergence_rate=True
    ):
        """
