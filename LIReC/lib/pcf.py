...
class PCFCalc:
    ...
    def __run_old__(self: PCFCalc, **kwargs) -> PCFCalc.Result or Exception:
        '''
        Approximate the value of this PCF. Will first calculate to an initial depth,
        and then will procedurally double the depth until the desired precision is obtained.
        ...
        kwargs = {**DEFAULTS, **kwargs}
        # TODO starting at some depth, testing run and __run_old__ on the same pcf yields weird stuff,
        # and increasing the precision seems to delay the depth that stuff starts getting weird.
        # the difference seems to happen only in last_matrix itself, as the actual value shows little variance.
        # i'm still not sure how much this matters, but this is why i'm leaving __run_old__ in for a commit, so we can figure this out later
        getcontext().prec = 2000
        mp.mp.dps = 2000
        ...
        while self.depth < kwargs['depth']:
            ...
            if self.depth % CALC_JUMP == 0:
                ...
                fr_list += [mp.log(mp.mpf(np.gcd(*self.mat[0,:]))) / mp.mpf(self.depth) + self.a.degree() * (1 - mp.log(self.depth))]
            if kwargs['timeout_sec'] and self.depth % kwargs['timeout_check_freq'] == 0 and time() - start > kwargs['timeout_sec']:
                break
            if self.depth == kwargs['depth']: # do we need to keep going?
                self.reduce()
                #print(fr_list)
