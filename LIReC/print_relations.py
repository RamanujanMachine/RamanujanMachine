...
def main():
    ...
    db = LIReC_DB()
    n = Symbol('n')
    for rel in db.session.query(Relation).order_by(Relation.precision.desc()):
        ...
        for const in rel.constants:
            named = db.session.query(NamedConstant).filter(NamedConstant.const_id == const.const_id).all()
            if named:
                toprint += f'    {named[0].name} : {named[0].description}'
            else:
                pcf = db.session.query(PcfCanonicalConstant).filter(PcfCanonicalConstant.const_id == const.const_id).all()
                if pcf:
                    pcf = pcf[0]
                    if pcf.original_a and pcf.original_b:
                        ...
                        if len(b_factors[1]) > 1: # usually b_n tends to have only nonpositive integer roots, so try to extract them!
                            b_string = reduce(add, [f'*({root.expr})**{mult}' if root.expr != n else f'*{root.expr}' for root, mult in b_factors[1]], f'{b_factors[0]}')
