...
def create_series_from_shift_reg(poly_a, initials, n):
    ...
    """
    assert (
        len(poly_a) - len(initials)
    ) == 1, "There should be deg(poly) initial conditions"
    assert (len(poly_a) > 0) and (
        poly_a[0] == 1
    ), "Illegal polynomial - first coefficient must be 1"
    a_ = []
...
class SignedRcfEnumeration(object):
    ...
    def create_rational_variations_enum(self):
        ...
        if self.do_print:
            print(
                "Number of variations before simplification is: {}".format(unsimplified)
            )
        start = time()
        ...
        if self.min_deg is not None:
            numerators = [
                list(numer)
                for numer in list(itertools.product(coeffs, repeat=self.poly_deg + 1))
                if len(numer) >= self.min_deg + 1
            ]
            denominators = [
                list(denom)
                for denom in list(itertools.product(coeffs, repeat=self.poly_deg + 1))
        ...
        for var in variations:
            if ((cnt % 1000) == 0) and (cnt != 0) and self.do_print:
                print(
                    "{} variations took {} minutes".format(
                        cnt, round((time() - mytimer) / 60, 2)
                    )
                )
            cnt += 1
            numer = var[0]
            ...
            if isinstance(
                var_sym, Rational
            ):  # Simplified expression is a rational number (independant of constant).
                continue
    ...
    def find_signed_rcf_conj(self):
        ...
        if self.custom_enum is None:
            ...
            strt = time()
            lhs = [
                var.subs({sympy.symbols("x"): self.const_sym})
        ...
        if self.do_print:
            print(
                "De-Facto Domain Size is: {}\n Starting preliminary search...".format(
                    domain_size
                )
            )
        ...
        for instance in itertools.product(lhs, sign_seqs):
            ...
            if (count % checkpoint == 0) and (self.do_print):
                print(
                    "\n{}% of domain searched.".format(
                        round(100 * count / domain_size, 2)
                    )
                )
                print("{} possible results found".format(len(inter_results)))
                print("{} minutes passed.\n".format(round((time() - start) / 60, 2)))
            b_ = sign_period * (
                (self.depth // seq_len) + 1
            )  # Concatenate periods to form sequence.
            b_ = b_[: self.depth]  # Cut to proper size.
            with mpmath.workdps(self.enum_dps):
                try:
                    signed_rcf = GeneralizedContinuedFraction.from_irrational_constant(
                        const_gen=var_gen, b_=b_
                    )
            ...
            if len(a_lfsr) < self.beauty_standard:
                inter_results.append(
                    [var, sign_period, a_[: (len(a_lfsr) - 1)], a_lfsr]
                )
    ...
    def print_results(self, results, latex=True):
        ...
        for res_num, res in enumerate(results):
            ...
            if not latex:
                ...
                print(
                    "Converged with a rate of {} digits per term".format(
                        mpmath.nstr(rate, 5)
                    )
                )
            else:
            ...
            if self.do_print:
                print(
                    "{} results were verified.\nThat took {}".format(
                        len(verified_results), end - start
                    )
                )
            # Print if requested:
            if self.do_print:
...
def esma_search_wrapper(
    ...
    if depth is not None:
        enum = SignedRcfEnumeration(
            ...
            cycle_len_range=cycle_range,
            depth=depth,
            coefficients_limit=coeff_lim,
            poly_deg=poly_deg,
            min_deg=min_deg,
