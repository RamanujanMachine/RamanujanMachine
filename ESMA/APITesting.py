...
class APITests(unittest.TestCase):
    def test_ESMA_api1(
        self,
    ):  # Test full enumeration and search configuration including saving binaries. Might ake a little longer
        cmd = (
            "ESMA -out_dir ./tmp -mode search -constant e -cycle_range 2 2 -depth 105 -poly_deg 1"
        ...
        files_there = os.path.exists("./tmp/res_list_0") and os.path.exists(
            "./tmp/recurring_by_value_0"
        )
    ...
    def test_ESMA_api2(self):  # Test standard build configuration.
        ...
        self.assertEqual(
            lhs,
            create_standard_lhs(
                poly_deg=1, coefficients_limit=1, do_print=(not args.no_print)
            ),
        )
    ...
    def test_ESMA_api3(
        self,
    ):  # Test search using an existing enumeration configuration.
        print("Creating and saving a temporary generic LHS enumeration.")
        custom_enum = create_standard_lhs(
            poly_deg=1, coefficients_limit=2, do_print=False
        )
        path = "./tmp"
        ...
        self.assertIn([(sympy.E / (sympy.E - 1)), [1, -1], [1, 0, -2, 0, 1]], adjusted)
        self.assertIn(
            [(sympy.E / (-2 + sympy.E)), [1, 1], [1, 0, 0, -1, 0, 0, -1, 0, 0, 1]],
            adjusted,
        )
        print("Search results are as expected.")
