...
def get_lhs_generator(generator_name, args):
    ...
    if generator_name == "biased_monoms":
        if (
            args.poly_deg <= 0
            or len(args.coeff_lim) != 2
            or args.coeff_lim[0] <= 0
            or args.coeff_lim[1] <= 0
        ):
            print(
                "Degree and limits must be positive. Expecting two coefficient limits."
            )
            raise AttributeError
        return lhs_generators.create_biased_monoms(
            args.poly_deg, args.coeff_lim[0], args.coeff_lim[1]
        )
    if generator_name == "standard":
        if (
            args.poly_deg <= 0
            or not isinstance(args.coeff_lim, int)
            or args.coeff_lim <= 0
        ):
            print(
            ...
            raise AttributeError
        return lhs_generators.create_standard_lhs(
            args.poly_deg, args.coeff_lim, args.out_dir, do_print=(not args.no_print)
        )
    ...
    if args.mode == "build":
        ...
        if args.out_dir is not None:
            if os.path.exists(args.out_dir):
                print(
                    "File under given name already exists. Choose different name for output file."
                )
                return
            else:
    ...
    if args.mode == "search":
        ...
        if args.lhs is not None:
            with open(args.lhs, "rb") as f:
                ...
                print(
                    "Loaded {} LHS variations. Took {} sec".format(
                        len(custom_lhs), time() - strt
                    )
                )
        else:
            custom_lhs = None
        ...
        if len(args.cycle_range) != 2 or args.cycle_range[0] > args.cycle_range[1]:
            ...
            raise ValueError
        results, _ = esma_search_wrapper(
            constant=g_const_dict[args.constant],
...
def main():
    ...
    if len(sys.argv) == 1:
        print(
            "You must input the running parameters to run the Ramanujan Machine. Please run 'python main.py \
